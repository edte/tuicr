...
| `minimal_ui`               | `false`      | Use delta-style file rules, boxed compact hunk labels, side-by-side gutters, blank separation for omitted context, and line-wise `j`/`k` viewport scrolling while hiding persistent app chrome. Command/search input and transient messages remain visible. |
